...
def test_action_output(mock_boto, mock_get_snapshot_ids):
    ...
    assert actions == {
        "image-20250101": api.Actions(
            ...
            policy={"action": "deprecate", "keep": 2, "expiration_days": None},
        )
    }
...
def test_deprecate_images_past_expiration(mock_boto, mock_get_snapshot_ids):
    ...
    config = configmodels.ConfigModel(
        **{
            "images": {
                "image-20250101": {"action": "deprecate", "keep": 3, "expiration_days": 90},
            },
            "options": {},
        }
    )
    ...
    assert actions == {
        "image-20250101": api.Actions(
            ...
            policy={"action": "deprecate", "keep": 3, "expiration_days": 90},
        )
    }
