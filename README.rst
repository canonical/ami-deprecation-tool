...
The third image (``some/image/path/image-C-$serial``) has a policy of ``{action: deprecate, keep 3, expiration_days 90}``. The three most recent images will not be scheduled for deprecation (like in the previous example). In addition, any image less than 3 months old, will also not be scheduled for deprecation. 
